program main
  ...
  integer(kind=4), parameter:: nvert=16
  ...
  character(kind=1,len=132) :: paramfile,glassfile,outfile
  logical:: fexist
  logical:: ispoisson
  ...
  call set_parameters(paramfile,ndisc,outfile,glassfile,ispoisson,0,mode)
  ...
  sigma0=(mdisc-mstar)/(2.*PI*rdisc*rdisc)
  write(*,*) 'Sigma0 (Gas Disc) [1e10 Msol/kpc^2]: ',sigma0
  call qromb(mdenc,dlog10(rtab(1)),dlog10(rtab(nrbin)),sum)
  write(*,*) 'Gas disc mass [1e10 Msol] :',sum
  molecular_weight=1.28
  cs=sqrt(kb*temp/mp/molecular_weight)*1d-3 ! in units of km/s
...
1111 format(a18,i4,4e18.8,a8,i3)
  ...
  do nr=1,nrbin
...
#endif         
     write(*,*)
     write(*,1111) 'Initial guess: ',nr,radius,radius/rdisc,rho0,sigma0*exp(-radius/rdisc)/(2*rho0)
     ...
     write(*,1111) 'Converged result: ',nr,radius,radius/rdisc,rho0,sigma0*exp(-radius/rdisc)/(2.*rho0),', niter: ',k
     ...
     do j=1,nzbin
        ...
        call mqromb(fintrp,10**lzmin,ztab(j),xgrid,ygrid,ngrid,sum2)
        mztab(nr,j)=sum2/sum
     end do
  ...
  do i=1,nrbin
     ...
     write(66,*) radius, sqrt(vcirc),sqrt(vdisc),sqrt(abs(vp))
  end do
  close(66)
  ...
  if(ispoisson) then
     ...
     do
        ...
        if(mtot.gt.mdisc_keep) exit

        ! Get the outer radius r1 -- inner radius given by r0
        i=1
        
        do
           if(mrtab(i).gt.mtot) exit
           i=i+1
        end do
        ...
        mvert=abs(zglass(nn))
        if(mvert.gt.1.0) mvert=1.0-abs(1.0-mvert)
        ...
        if(abs(z).gt.r1) print *, z,r1,nn,mvert
